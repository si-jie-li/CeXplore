...
- Drag, right-drag, and scroll in the 3D view to rotate, pan, and zoom. Open **Angle** to enter exact azimuth/elevation degrees or jump to ±AP/±LR/±VD orthogonal views while preserving the current focus target and zoom distance. The camera remains unchanged during playback; Reset, Focus, and Angle move it only when explicitly requested.
