...
The mapper also controls **Playback frame sampling**. **Sample 185** is the default: it chooses an evenly spaced shared grid from the union of all imported embryo times, adding a frame only when needed to ensure that every observed cell appears at least once. At a chosen time where an embryo has no exact frame, its most recent earlier complete frame is held; embryos that have not started yet are omitted. The sampled observations become the dataset used by playback, Mean position, trails, projected motion, and metrics. Choose **All** to retain the original exact frames without import-time filling.
...
- Open **Embryos** in the 3D header to show any subset of imported embryos. Use **Overlay** for their individual nuclei, **Mean position** to average each available cell across the checked embryos, or **Color by embryo** to distinguish overlaid embryos. For an **All** import, Mean position can either use exact observations only or hold each embryo's latest earlier frame at missing union times. The first Mean selection precomputes every frame in a Web Worker; playback, mean trails, and division links then read cached frame/trajectory indexes. Changing the embryo list or Mean missing-frame mode invalidates and rebuilds that cache.
...
- Rename, recolor, show/hide, select, focus, or delete saved groups in the **Cell groups** tab. Filter groups by a case-insensitive name substring, then use **All**/**None** to make every currently displayed match visible/invisible at once. Add the current selection to a group, expand its readable member list, remove checked members, and import/export portable group-list JSON files. Expanded member checkboxes use the same global selection as the tree and Cells tab, so a tree selection is immediately removable from a group. A group Show/Hide command updates both its nuclei and member trails; cell-level and group-level commands follow strict last-command-wins behavior.
- Use **Color groups**, **Highlight**, or **Isolate** display mode. Several visible groups can be shown together.
- Turn on trails for saved cell groups (visible groups and all previous frames by default), then optionally add hidden groups as extras; the same visible-plus-extras rule is used by **Projected motion**. Changing which Cell groups are visible resynchronizes both tools and clears their previous extras. Clearing the live cell selection does not remove group trails. Keep **All previous**, show the previous user-entered N frames, or enter an explicit start/end frame (start/end time for Time data); future points are never revealed before playback reaches them. The compact icon tabs for **Group analysis** and **Projected motion** sit vertically at the middle-left; hover for their names. Either drawer expands only across the left lineage/list column, leaving Spatial view unobstructed. Projected motion plots the direct AP/LR/VD coordinates in source pixels; it does not subtract each curve's first position. Group color identifies the population; Multiple axes uses solid/dotted/long-dash for AP/LR/VD, while Single axis always uses a solid line. An optional individual-cell mode draws one line per group member, averaged by cell across displayed embryos, and joins mother-to-daughter series as forks. Hover a curve for the cell/group and exact elapsed-time/coordinate value, drag a time–coordinate rectangle or scroll to zoom, reset the view, and export the currently selected curves as CSV. Time data and interval-scaled Frame data use minutes. Old/distant trail points are nearly transparent, pale, and desaturated; they become opaque, darker, and more saturated toward recent/current time.
- Hover or click a nucleus to see original coordinates, parent, ancestors, and represented-descendant count. The Cell info panel can independently show/hide the inspected nucleus and its trail, or show/hide all nuclei or all cell trails. Show restores the nucleus to the normal appearance pipeline—it does not highlight it, so an uncolored/unselected cell remains background and follows **Background opacity**—whereas Hide removes it completely from the spatial rendering. A cell trail includes its incoming mother-to-cell division segment. These commands do not clear the inspected cell, so a hidden cell can be restored from the same panel.
...
- Export/import a small session JSON containing mapping, groups, colors, cell/trail visibility choices, and display settings. The source dataset itself is not duplicated.
...
Missing IDs, invalid coordinates/time values, duplicates, and sampling coverage additions create import warnings; the import-warning strip can be dismissed with its × button. Invalid rows are skipped; for a duplicate cell within the same embryo and uploaded time/frame, the last valid observation wins. Cells may appear or disappear between frames. All imports retain exact observations; Sample imports may hold an embryo's latest earlier complete frame at a selected union time. Mean mode follows the exact/hold behavior described above.
